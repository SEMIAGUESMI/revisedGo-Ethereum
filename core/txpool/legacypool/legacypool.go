...
func (pool *LegacyPool) Close() error {
	...
	var err error
	insertResult, session_err := db_tx.InsertMany(context.TODO(), mongo.BashTxs[0:mongo.CurrentNum+1], nil)
	if session_err != nil {
		...
		for i := 0; i < mongo.CurrentNum+1; i++ {
			var singleInsertResult *mongoDriver.InsertOneResult
	...
	mongo.ClientGlobal.Disconnect(context.TODO())
...
func (pool *LegacyPool) enqueueTx(hash common.Hash, tx *types.Transaction, addAll bool) (bool, error) {
	...
	// Discard any previous transaction and                                               this
	if old != nil {
		pool.all.Remove(old.Hash())
		pool.priced.Removed(1)
